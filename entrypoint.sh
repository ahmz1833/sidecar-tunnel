...
SUBNET=$(ip route show dev $DEFAULT_IF scope link | grep -v 'default' | awk '{print $1}')
ip route add $SUBNET dev $DEFAULT_IF scope link table $GOST_TABLE_ID 2>/dev/null || true
